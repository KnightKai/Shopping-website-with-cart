...
router.get('/sortup',(req,res)=>{
    Prod.find().sort({price: 1}).exec(function(err,cursor){
        var productChunks = [];
        var chunkSize = 3;
        for (var i = 0; i < cursor.length; i += chunkSize) {
            productChunks.push(cursor.slice(i, i + chunkSize));
        }
